...
export default function EmailDeliverabilityPanel({ token }: { token: string }) {
  ...
  return (
    <section className="app-card mt-6 p-4">
      <h2 className="text-sm font-semibold text-[var(--text-strong)]">Entregabilidade de emails</h2>
      <p className="mt-1 text-xs text-[var(--text-muted)]">Taxa de sucesso/falha de envio por template — os destinatários não são armazenados, apenas um hash.</p>
