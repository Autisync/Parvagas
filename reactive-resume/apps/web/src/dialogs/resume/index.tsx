...
export function CreateResumeDialog(_: DialogProps<"resume.create">) {
	...
	const { mutate: createFromParvagasProfile, isPending: isImportingFromParvagas } = useMutation(
		orpc.resume.createFromParvagasProfile.mutationOptions(),
	);
	...
	const onCreateFromParvagasProfile = () => {
		const values = form.state.values;
		const randomName = generateRandomName();
		const profileName = values.name || randomName;
		const profileSlug = values.slug || slugify(profileName);

		const toastId = toast.loading(t`Importing your Parvagas profile...`);
		createFromParvagasProfile(
			{
				name: profileName,
				slug: profileSlug,
				tags: values.tags,
		...
		<DialogContent>
			...
			>
				...
				<DialogFooter>
					...
					>
						<Button type="submit" disabled={isPending || isImportingFromParvagas}>
							<Trans>Create</Trans>
						</Button>
						...
						<DropdownMenu>
							<DropdownMenuTrigger
								render={
									<Button size="icon" disabled={isPending || isImportingFromParvagas}>
										<CaretDownIcon />
									</Button>
								}
							/>
							...
							<DropdownMenuContent align="end" className="w-fit">
								...
								<DropdownMenuItem onClick={onCreateFromParvagasProfile}>
									<Trans>Criar a partir do meu perfil Parvagas</Trans>
								</DropdownMenuItem>
							</DropdownMenuContent>
						</DropdownMenu>
					</ButtonGroup>
