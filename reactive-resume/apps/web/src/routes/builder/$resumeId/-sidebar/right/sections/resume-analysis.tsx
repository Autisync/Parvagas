												...
												{suggestion.exampleRewrite && (
													<div className="space-y-2">
														...
														<div className="flex gap-2">
															<ApplyRewriteButton example={suggestion.exampleRewrite} />
															<ServerRewriteButton resumeId={resume.id} />
														</div>
													</div>
												)}
											</div>
...
function ApplyRewriteButton({ example }: { example: string }) {
	const updateResumeData = useUpdateResumeData();

	const onApply = () => {
		updateResumeData((draft) => {
			// insert as a simple paragraph; the editor expects HTML in summary.content
			draft.summary.content = `<p>${escapeHtml(example)}</p>`;
		});

		toast.success(t`Applied rewrite to Summary.`);
	};

	return (
		<Button size="sm" variant="outline" onClick={onApply} className="ml-auto">
			<Trans>Apply to Summary</Trans>
		</Button>
	);
}

function ServerRewriteButton({ resumeId }: { resumeId: string }) {
	const prompt = usePrompt();
	const updateResumeData = useUpdateResumeData();
	const queryClient = useQueryClient();

	const mutation = useMutation({
		...orpc.resume.rewrite.mutationOptions(),
		onSuccess: (data) => {
			// update resume cache
			queryClient.setQueryData(orpc.resume.getById.queryKey({ input: { id: resumeId } }), (old: unknown) => {
				if (!old || typeof old !== "object") return old;

				const current = old as { name?: string; data?: unknown };
				return { ...current, name: data.title ?? current.name, data: current.data };
			});

			// apply to draft (summary + title)
			updateResumeData((draft) => {
				if (data.summary) draft.summary.content = `<p>${escapeHtml(data.summary)}</p>`;
				if (data.title) draft.title = data.title;
			});

			toast.success(t`Rewrite applied and saved.`);
		},
		onError: (err) => {
			toast.error(t`Failed to rewrite resume.`, { description: getOrpcErrorMessage(err) });
		},
	});

	const onServerRewrite = async () => {
		const tone =
			(await prompt(t`Tone`, {
				defaultValue: "professional",
				description: t`Choose a tone for the rewrite (e.g., professional, friendly, concise).`,
			})) ?? "professional";
		const instructions =
			(await prompt(t`Instructions (optional)`, {
				defaultValue: "",
				description: t`Any special instructions for the rewrite?`,
			})) ?? undefined;

		mutation.mutate({ id: resumeId, tone, instructions });
	};

	return (
		<Button size="sm" variant="ghost" onClick={onServerRewrite} className="ml-auto">
			<Trans>Rewrite (server)</Trans>
		</Button>
	);
}
