...
describe("Header", () => {
	it("renders a homepage link with the brand icon", () => {
		...
		expect(home?.getAttribute("aria-label")).toBe("Parvagas CV Builder - Go to homepage");
	});

	it("renders Parvagas CV Builder navigation and login action", () => {
		const { container } = renderHeader();
		const dashboard = Array.from(container.querySelectorAll("a")).find((a) => a.getAttribute("href") === "/dashboard/resumes");
		const parvagasLogin = Array.from(container.querySelectorAll("a")).find((a) => a.getAttribute("href") === "/auth/parvagas/start");
		expect(dashboard).toBeDefined();
		expect(parvagasLogin).toBeDefined();
	});
