...
export function DonationToast() {
	...
	const showToast = useCallback(() => {
		...
		const onDonate = (t: string | number) => {
			...
			window.open("https://parvagas.pt", "_blank", "noopener,noreferrer");
		};
			...
			<div className="flex items-start gap-3">
				...
				<div className="min-w-0 flex-1 space-y-1">
					<p className="font-semibold text-sm tracking-tight">
						<Trans>Need help with CV Builder?</Trans>
					</p>
					<p className="text-pretty text-muted-foreground text-xs">
						<Trans>Parvagas support can help with account, import, and resume publishing issues.</Trans>
					</p>
				</div>
			</div>
