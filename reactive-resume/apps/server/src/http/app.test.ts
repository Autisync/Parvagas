...
const mocks = vi.hoisted(() => ({
	handleAuth: vi.fn(),
	handleOAuth: vi.fn(),
	handleParvagasStart: vi.fn(),
	handleParvagasExchange: vi.fn(),
	handleRpc: vi.fn(),
	handleOpenApi: vi.fn(),
	handleHealth: vi.fn(),
	handleUpload: vi.fn(),
...
vi.mock("./parvagas-auth", () => ({
	handleParvagasStart: mocks.handleParvagasStart,
	handleParvagasExchange: mocks.handleParvagasExchange,
}));
...
beforeEach(() => {
	...
	mocks.handleParvagasStart.mockResolvedValue(new Response("parvagas-start"));
	mocks.handleParvagasExchange.mockResolvedValue(new Response("parvagas-exchange"));
...
describe("createApp", () => {
	...
	it("routes /auth/parvagas/exchange before the static web fallback", async () => {
		const { createApp } = await import("./app");
		const app = createApp();
		const request = new Request("http://localhost:3001/auth/parvagas/exchange?code=abc");

		const response = await app.fetch(request);

		await expect(response.text()).resolves.toBe("parvagas-exchange");
		expect(mocks.handleParvagasExchange).toHaveBeenCalledWith(request);
		expect(mocks.serveWebDistStatic).not.toHaveBeenCalled();
		expect(mocks.handleWebApp).not.toHaveBeenCalled();
	});
