...
def test_exhausted_when_ingest_count_reaches_cap():
    ...
    assert tasks._scrape_budget_exhausted(
        ingested=tasks._FALLBACK_SCRAPER_MAX_INGEST_PER_RUN, started_at=started, now=now
    ) is True


def test_exhausted_when_wall_clock_budget_elapses():
    started = datetime(2026, 7, 1, 12, 0, 0)
    now = started + timedelta(seconds=tasks._FALLBACK_SCRAPER_RUN_BUDGET_SECONDS)
...
def test_budgets_are_finite_positive_numbers():
    ...
    assert tasks._FALLBACK_SCRAPER_MAX_INGEST_PER_RUN > 0
    assert tasks._FALLBACK_SCRAPER_RUN_BUDGET_SECONDS > 0
