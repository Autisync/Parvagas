...
class CVBuilderAuthCode(Base, TimestampMixin):
    """One-time authorization code for Parvagas -> CV Builder SSO exchange."""
    __tablename__ = "cv_builder_auth_codes"

    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    code_hash = Column(String(128), nullable=False, unique=True, index=True)
    audience = Column(String(64), nullable=False, default="cv-builder")
    user_id = Column(String(36), ForeignKey("users.id"), nullable=False, index=True)
    nonce = Column(String(128), nullable=False)
    return_url = Column(String(1024), nullable=False)
    target_resume_id = Column(String(255), nullable=True)
    expires_at = Column(DateTime, nullable=False)
    used_at = Column(DateTime, nullable=True)
