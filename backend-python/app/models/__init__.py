...
class SSOHandoffCode(Base, TimestampMixin):
    """Short-lived bridge code minted by an authenticated (bearer-token)
    frontend request, then carried on a full-page redirect into
    app.api.v1.resume_sso's /oauth/authorize.

    Needed because Parvagas auth is pure bearer-token (JWT in localStorage,
    no cookies) — a real OIDC /authorize hit arrives as a browser GET
    redirect from Reactive Resume, which can't carry an Authorization
    header. The frontend mints this first via an authenticated fetch, then
    navigates with the code attached. ~60s TTL, single-use.
    """
    __tablename__ = "sso_handoff_codes"
...
class OAuthAuthorizationCode(Base, TimestampMixin):
    """Proper OIDC authorization code, minted by /oauth/authorize after it
    consumes an SSOHandoffCode, and exchanged server-to-server by Reactive
    Resume at /oauth/token. Single-client allow-list (RESUME_SSO_CLIENT_ID) —
    no OAuth client registry table, since Reactive Resume is the only
